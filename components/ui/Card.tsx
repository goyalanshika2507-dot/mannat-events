  ...
  return (
    <As
      className={cn(
        ...
        hover && 'hover:shadow-[0_12px_38px_rgba(26,26,26,0.06)] hover:-translate-y-1',
        className
      )}
    >
      {children}
...
export function CardHeader({ title, description, action, eyebrow }: CardHeaderProps) {
  return (
    <div className="flex items-start justify-between gap-4 mb-6">
      <div>
        {eyebrow && (
          <p className="text-caption mb-1">{eyebrow}</p>
        )}
...
export function StatCard({ label, value, className, accent = false }: StatCardProps) {
  return (
